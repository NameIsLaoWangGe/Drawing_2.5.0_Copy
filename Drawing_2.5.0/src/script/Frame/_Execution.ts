import lwg, { EventAdmin } from "./Lwg";
    ...
    */
    export function _createAddExecution(x, y, func): void {
        ...
        Laya.loader.load('prefab/execution.json', Laya.Handler.create(this, function (prefab: Laya.Prefab) {
            ...
            sp.zOrder = 50;
            if (_ExecutionNode) {
                lwg.Animation2D.move_Simple_01(sp, sp.x, sp.y, _ExecutionNode.x, _ExecutionNode.y, 800, 100, f => {
                    lwg.Animation2D.fadeOut(sp, 1, 0, 200, 0, f => {
                        lwg.Animation2D.upDwon_Shake(_ExecutionNode, 10, 80, 0, null);
                        if (func) {
                            func();
                        }
                    });
    ...
    export function createConsumeEx(subEx): void {
        ...
        lwg.Animation2D.fadeOut(label, 0, 1, 200, 150, f => {
            lwg.Animation2D.leftRight_Shake(_ExecutionNode, 15, 60, 0, null);
...
export class Execution extends lwg.Admin._Object {
    Num: Laya.FontClip;
    CountDown: Laya.Label;
    CountDown_board: Laya.Label;

    lwgOnAwake(): void {
        this.Num = this.owner.getChildByName('Num') as Laya.FontClip;
        this.CountDown = this.owner.getChildByName('CountDown') as Laya.Label;
        this.CountDown_board = this.owner.getChildByName('CountDown_board') as Laya.Label;
        this.countNum = 59;
